...
switch ($verb) {
    ...
    case 'create':
        if(isset($_POST['name']) && trim($_POST['name']) != "") {
            ...
            $category_other = isset($_POST['category_other']) ? $_POST['category_other'] : ""; // string
            $weeks = $_POST['weeks']; // int
            $amount = $_POST['amount']; // int
            $funding_type = $_POST['funding_type']; // int
            $contributing = isset($_POST['contributing']) ? $_POST['contributing'] : 0; // int
            $asking = isset($_POST['asking']) ? $_POST['asking'] : 0; // int
            $professor_id = $_POST['professor_id']; // int
            $details = $_POST['details']; // string

            // Details of contact person
            $contact_name = isset($_POST['contact_name']) ? $_POST['contact_name'] : ""; // string
            $sex = $_POST['sex']; // int; MALE: 1, FEMALE: 2
            $age = $_POST['age']; // int
            $occupation = isset($_POST['occupation']) ? $_POST['occupation'] : ""; // string
            $contact_address = isset($_POST['contact_address']) ? $_POST['contact_address'] : ""; // string
            $gps = isset($_POST['gps']) ? $_POST['gps'] : ""; // string
            $phone = $_POST['phone']; // int
            $email = isset($_POST['email']) ? $_POST['email'] : ""; // string

            $data = Project::create($name, $group_id, $address, $category, $category_other, $weeks, $amount, $funding_type, $contributing, $asking, $professor_id, $details,
